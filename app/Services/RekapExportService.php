...
class RekapExportService
{
    ...
    public static function exportRekapSiswa(): StreamedResponse
    {
        ...
        foreach ($kelasList as $kelas) {
            ...
            static::mergeSubtitle($sheet, "A2:{$lastCol}2",
            ...
            $sheet->getStyle("A4:I4")->applyFromArray([
                'font' => ['size' => 8],'fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => 'EFF6FF']],
            ]);
            ...
            foreach ($siswaList as $idx => $siswa) {
                ...
                foreach (['A','E','F','G','H'] as $c) { $sheet->getStyle("{$c}{$row}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER); }
                $sheet->getStyle("A{$row}:{$lastCol}{$row}")->applyFromArray(['borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['rgb' => 'E5E7EB']]]]);
                if ($idx % 2 === 1) { $sheet->getStyle("A{$row}:D{$row}")->applyFromArray(['fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => 'F8FAFC']]]); }
                $row++;
            }
            ...
            $legRow = $row + 2;
            $sheet->setCellValue("A{$legRow}", 'Keterangan:'); $sheet->getStyle("A{$legRow}")->getFont()->setBold(true);
            $sheet->setCellValue("B{$legRow}", '✓ = Terverifikasi'); $sheet->setCellValue("C{$legRow}", '⏳ = Menunggu');
            $sheet->setCellValue("D{$legRow}", '✗ = Ditolak'); $sheet->setCellValue("E{$legRow}", '— = Belum Lapor');

            foreach (['A'=>5,'B'=>28,'C'=>14,'D'=>10,'E'=>9,'F'=>13,'G'=>11,'H'=>9,'I'=>9] as $c => $w) { $sheet->getColumnDimension($c)->setWidth($w); }
            $sheetIndex++;
        }
    ...
    public static function exportDetailSiswa(User $siswa): StreamedResponse
    {
        ...
        $isMuslim    = !in_array(strtolower($siswa->agama ?? ''), ['kristen','katolik','hindu','budha','konghucu']);
        ...
        static::mergeSubtitle($sheet, "A2:{$lastColRange}2",
        ...
        $sheet->setCellValue('A4', 'RINGKASAN'); $sheet->getStyle('A4')->applyFromArray(['font' => ['bold' => true, 'size' => 11]]);
        foreach ([['Total Laporan',$total],['Terverifikasi',$verified],['Menunggu',$pending],['Ditolak',$rejected],['Belum Lapor',$belum],['Kepatuhan',"{$rate}%"]] as $i => [$l,$v]) {
            $r = 5 + $i;
            $sheet->setCellValue("A{$r}", $l); $sheet->setCellValue("B{$r}", $v);
        ...
        foreach ($headers as $col => $label) { $sheet->setCellValue("{$col}{$tableStartRow}", $label); }
        ...
        for ($d = 1; $d <= $maxHari; $d++) {
            ...
            if ($d % 2 === 0) { $sheet->getStyle("A{$dataRow}:B{$dataRow}")->applyFromArray(['fill' => ['fillType' => Fill::FILL_SOLID, 'startColor' => ['rgb' => 'F8FAFC']]]); }
        ...
        foreach ($wrapCols as $col) {
            for ($r = $tableStartRow + 1; $r < $dataRow; $r++) { $sheet->getStyle("{$col}{$r}")->getAlignment()->setWrapText(true); }
        }
    ...
    {
        $puasa = $data['puasa'] ?? '';
        $sheet->setCellValue("D{$row}", match($puasa) {'ya'=>'Ya','tidak'=>'Tidak',default=>'—'});
        static::colorBool($sheet, "D{$row}", $puasa === 'ya');

        $sholat = $data['sholat'] ?? [];
        foreach (['subuh'=>'E','dzuhur'=>'F','ashar'=>'G','maghrib'=>'H','isya'=>'I'] as $key => $col) {
            $val = $sholat[$key] ?? '';
            $label = match($val) {'jamaah'=>'Jmh','munfarid'=>'Mfr','tidak'=>'Tdk',default=>'—'};
            $sheet->setCellValue("{$col}{$row}", $label);
            $bg = match($val) {'jamaah'=>'DCFCE7','munfarid'=>'FEF9C3','tidak'=>'FEE2E2',default=>'F1F5F9'};
            $fg = match($val) {'jamaah'=>'166534','munfarid'=>'92400E','tidak'=>'991B1B',default=>'94A3B8'};
        ...
        $sheet->setCellValue("J{$row}", match($tarawih) {'jamaah'=>'Jmh','munfarid'=>'Mfr','tidak'=>'Tdk',default=>'—'});
        $bg = match($tarawih) {'jamaah'=>'DCFCE7','munfarid'=>'FEF9C3','tidak'=>'FEE2E2',default=>'F1F5F9'};
        $fg = match($tarawih) {'jamaah'=>'166534','munfarid'=>'92400E','tidak'=>'991B1B',default=>'94A3B8'};
        ...
        foreach ($kegiatanLabels as $key => $label) {
            if (!empty($kegiatan[$key]) && $kegiatan[$key] !== false && $kegiatan[$key] !== 'tidak') { $done[] = $label; }
        }
        ...
        $sheet->setCellValue("P{$row}", match($cMode) {'offline'=>'Offline','online'=>'Online','tidak'=>'Tidak',default=>'—'});
    ...
    private static function fillNonMuslimRow($sheet, int $row, array $data): void
    {
        ...
        $kegiatan = $data['kegiatan'] ?? [];
        $done = [];
        foreach ($kegiatanLabels as $key => $label) { if (($kegiatan[$key] ?? '') === 'ya') { $done[] = $label; } }
    ...
    {
        $letter = '';
        while ($index >= 0) { $letter = chr($index % 26 + 65) . $letter; $index = intval($index / 26) - 1; }
        return $letter;
    }
    ...
    private static function streamDownload(Spreadsheet $spreadsheet, string $filename): StreamedResponse
    {
        ...
        return response()->streamDownload(function () use ($writer) { $writer->save('php://output'); }, $filename, ['Content-Type' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet']);
    }
}
