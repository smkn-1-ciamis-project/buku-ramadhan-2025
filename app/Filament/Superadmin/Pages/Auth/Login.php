...
{
  protected static string $view = 'filament.superadmin.pages.auth.login';
  ...
  public function authenticate(): ?LoginResponse
  {
    ...
    // Super Admin boleh multi-device, skip single-session pre-check
    ...
    // Verify user has Super Admin role — prevent unauthorized session side-effects
    if (! $user->canAccessPanel(\Filament\Facades\Filament::getCurrentPanel())) {
      Auth::logout();
      session()->invalidate();
      session()->regenerateToken();
      $this->errorPopupMessage = 'Akun Anda tidak memiliki akses ke panel Super Admin.';
      $this->showErrorPopup = true;
      return null;
    }
